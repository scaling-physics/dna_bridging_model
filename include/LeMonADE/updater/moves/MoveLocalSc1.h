...
public:
    ...
    template <class IngredientsType> void init(const IngredientsType& ing, bool bridge);
...
template <class IngredientsType>
void MoveLocalSc::init(const IngredientsType& ing, bool bridge)
{
    this->resetProbability();
    ...
    //draw direction
    uint32_t randomDir=this->randomNumbers.r250_rand32() % 6;
    VectorInt3 vertexPos(ing.getMolecules()[id]);

    // Set probability of bridging
    this-> multiplyProbability(exp(-vertexPos[0]*vertexPos[0]));

    VectorInt3 partnerPos[6];

    std::cout<<vertexPos<<std::endl;
    partnerPos = vertexPos + steps[randomDir];

   std::cout<<partnerPos<<std::endl;

    for (uint32_t n = 0;n<6;n++) {
        partnerPos[n] = vertexPos + steps[n] + steps[n];
        for (uint32_t m = 0; m < ing.getMolecules().size(); m++)
        {
            VectorInt3 a(ing.getMolecules()[m]);
            if (partnerPos[n] == a)
                std::cout << a<< std::endl;
        }
    }
    ...
    //            ing.modifyMolecules().setBridges(id, m);
//    }
//    this->setDir(steps[randomDir]);
