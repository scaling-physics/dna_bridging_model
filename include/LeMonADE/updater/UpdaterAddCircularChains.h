 ...
 * @file
 *
 * @class UpdaterAddCircularChains
 *
...
class UpdaterAddCircularChains: public UpdaterAbstractCreate<IngredientsType>
{
  typedef UpdaterAbstractCreate<IngredientsType> BaseClass;

public:
  UpdaterAddCircularChains(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, int32_t type1_=1, int32_t type2_=2, bool IsSolvent=false);
...
*/
template < class IngredientsType >
UpdaterAddCircularChains<IngredientsType>::UpdaterAddCircularChains(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, int32_t type1_, int32_t type2_, bool IsSolvent_):
...
*/
template < class IngredientsType >
void UpdaterAddCircularChains<IngredientsType>::initialize(){
  std::cout << "initialize UpdaterAddCircularChains" << std::endl;
...
*/
template < class IngredientsType >
bool UpdaterAddCircularChains<IngredientsType>::execute(){
  if(wasExecuted)
    return true;

  std::cout << "execute UpdaterAddCircularChains" << std::endl;

    for(uint32_t i=0;i<(NChain);i++){
        for(uint32_t j=0;j<(NMonoPerChain-59);j++){
            if(j==0)
                addSingleMonomer(type2);
            else{

//	if(ingredients.getMolecules()[ingredients.getMolecules().size()-1].getAttributeTag() == type1)
                if(j==NMonoPerChain/2-1)
...
*/
template < class IngredientsType >
void UpdaterAddCircularChains<IngredientsType>::cleanup(){
