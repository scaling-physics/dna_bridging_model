...
private:
  ...
  //! Calculate spatially varying probabiltiy
  double getSpatialProbabilityFactor(int32_t typeA,
                                     int32_t typeB, int32_t x) const;
...
public:
  ...
  //!sets mean and std of gaussian
  void setGaussian(int32_t mean, int32_t std);
 ...
 * is only implemented for safety purposes.
 *
 * @param [in] ingredients A reference to the IngredientsType - mainly the system
    ...
    const MoveLocalSc& move) const
{
    ...
    //with the probability, for contacts taken away the probability is divided.
    ...
    actual+=direction;
    actual-=perp1;
    prob*= getSpatialProbabilityFactor(monoType,int32_t(ingredients.getLatticeEntry(actual)),actual.getX());
    actual+=perp2;
    prob*= getSpatialProbabilityFactor(monoType,int32_t(ingredients.getLatticeEntry(actual)),actual.getX());
    ...
    actual-=direction;
    actual-=perp1;
    prob_div*= getSpatialProbabilityFactor(monoType,int32_t(ingredients.getLatticeEntry(actual)),actual.getX());
    actual+=perp2;
    prob_div*= getSpatialProbabilityFactor(monoType,int32_t(ingredients.getLatticeEntry(actual)),actual.getX());
    actual=actual+perp2+perp1;
    prob_div*= getSpatialProbabilityFactor(monoType,int32_t(ingredients.getLatticeEntry(actual)),actual.getX());
    ...
    if(0<typeA && typeA<=255 && 0<typeB && typeB<=255)
      {
        ...
        std::cout<<"set interaction between types ";
...
template<template<typename> class LatticeClassType>
double FeatureNNInteractionSc<LatticeClassType>::getSpatialProbabilityFactor(int32_t typeA,
                                                                             int32_t typeB,int32_t x) const {
    double prob;
    double d =1.0*x;
    if (interactionTable[typeA][typeB]>0.01)
    prob = 1/(std*2*3.14)*exp(-0.5*pow((d-mean)/std,2));
    else
        prob = 1.0;
    return prob;
}
