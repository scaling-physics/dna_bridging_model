...
#ifndef LEMONADE_ANALYZER_POSITION_OF_TER_H
#define LEMONADE_ANALYZER_POSITION_OF_TER_H
...
/*************************************************************************
 * definition of AnalyzerPositionOfTer class
 * ***********************************************************************/

/**
 * @file
 *
 * @class AnalyzerPositionOfTer
 *
 ...
 * The default output filename is PositionOfTerTimeSeries.dat and it can be changed by argument
 ...
 * also write a new analyzer, inheriting from AnalyzerPositionOfTer, and
 * overwriting the initialize function.
 */
template < class IngredientsType > class AnalyzerPositionOfTer : public AbstractAnalyzer
{
...
private:
    ...
    std::vector< std::vector<double> > PositionOfTerTimeSeries;
    ...
    //! save the current values in PositionOfTerTimeSeriesX, etc., to disk
    void dumpTimeSeries();
...
public:
    ...
    //! constructor
    AnalyzerPositionOfTer(const IngredientsType& ing,
                             std::string filename="PositionOfTerTimeSeries.dat",uint32_t terC=0);

    //! destructor. does nothing
    virtual ~AnalyzerPositionOfTer(){}
 ...
 * @param filename output file name. defaults to "PositionOfTerTimeSeries.dat".
 * */
template<class IngredientsType>
AnalyzerPositionOfTer<IngredientsType>::AnalyzerPositionOfTer(
        const IngredientsType& ing,
        std::string filename, uint32_t terC)
        :ingredients(ing)
        ,ter(terC)
        ,PositionOfTerTimeSeries(4,std::vector<double>(0))
        ,bufferSize(100)
        ,outputFile(filename)
 ...
 * */
template< class IngredientsType >
void AnalyzerPositionOfTer<IngredientsType>::initialize()
{
 ...
 * */
template< class IngredientsType >
bool AnalyzerPositionOfTer<IngredientsType>::execute()
{
    ...
    PositionOfTerTimeSeries[0].push_back(ingredients.getMolecules()[ter].getX());
    PositionOfTerTimeSeries[1].push_back(ingredients.getMolecules()[ter].getY());
    PositionOfTerTimeSeries[2].push_back(ingredients.getMolecules()[ter].getZ());
    PositionOfTerTimeSeries[3].push_back(ter);
...
template<class IngredientsType>
void AnalyzerPositionOfTer<IngredientsType>::cleanup()
{
    std::cout<<"AnalyzerPositionOfTer::cleanup()...";
...
/**
 * @details Saves the current content of PositionOfTerTimeSeries to the file
 * PositionOfTerTimeSeries.dat. The output format is:
 * mcs Rg2_x Rg2_y Rg2_z Rg2_tot
 * */
template<class IngredientsType>
void AnalyzerPositionOfTer<IngredientsType>::dumpTimeSeries()
{
    //fist make a single vector<vector<double> > for writing the results
    std::vector<std::vector<double> > resultsTimeseries=PositionOfTerTimeSeries;
    ...
    if(isFirstFileDump){
        ...
        commentTimeSeries<<"Created by AnalyzerPositionOfTer\n";
    ...
    MCSTimes.resize(0);
    PositionOfTerTimeSeries.resize(0);
    PositionOfTerTimeSeries.resize(4,std::vector<double>(0));
}
 ...
 * */
template<class IngredientsType>
VectorDouble3 AnalyzerPositionOfTer<IngredientsType>::calculateRg2Components(
